#!/bin/bash
if [ ! -f /tmp/server_wp_all_update ] && [ -f /home/vpsscript.conf ]; then
. /home/vpsscript.conf
fi
...
update_default_htaccess(){
	yes | cp -rf /etc/vpsscript/menu/tienich/.htaccess $1/.htaccess
					...
					if [ ! "$4" == "" ]; then
						...
						if [ $? -eq 0 ]; then
							...
							if [ -f /etc/init.d/directadmin ]; then
								...
								# voi vpsscript, HOCVPS -> phan quyen cho user va nginx
								id -u nginx
								if [ $? -eq 0 ]; then
